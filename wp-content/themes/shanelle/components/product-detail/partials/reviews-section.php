...
$review_unavailable_message  = isset( $review_unavailable_message ) ? (string) $review_unavailable_message : '';
?>
<section
...
>
	<header class="product-reviews__header">
		<h2 id="<?php echo esc_attr( $heading_id ); ?>" class="product-reviews__title product-detail__section-title">
			...
			if ( $review_count > 0 ) {
				printf(
					/* translators: %d: exact WooCommerce review count */
	...
	<div id="reviews" class="product-reviews__content" tabindex="-1" aria-labelledby="<?php echo esc_attr( $heading_id ); ?>">
	...
	<?php elseif ( '' !== $review_unavailable_message ) : ?>
		<p class="product-reviews__verification-required text-body-sm text-muted" role="status">
			<?php echo esc_html( $review_unavailable_message ); ?>
		</p>
	<?php endif; ?>
</section>
