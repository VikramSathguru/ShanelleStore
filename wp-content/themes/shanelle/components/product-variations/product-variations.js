...
/**
 * Apply APG roving tabindex within a variation radiogroup.
 *
 * @param {HTMLElement} group
 */
function syncRovingTabindex( group ) {
	const options = Array.from( group.querySelectorAll( '[data-shanelle-variation-option]' ) )
	...
	const selected = options.find( ( option ) => option.classList.contains( 'is-selected' ) && ! option.disabled );
...
/**
 * Set a native WC attribute select and fire the jQuery change WC listens for.
 * Native Event('change') does not reliably reach `.on('change.wc-variation-form')`.
 *
...
function clearSelections( root ) {
	...
	const selects = root.querySelectorAll( '.variations select, [data-shanelle-variation-native] select' );

	if ( typeof window.jQuery !== 'undefined' && selects.length ) {
		// One jQuery batch so WC onChange → check_variations → reset_data runs.
		window.jQuery( selects ).val( '' ).trigger( 'change' );
	} else {
