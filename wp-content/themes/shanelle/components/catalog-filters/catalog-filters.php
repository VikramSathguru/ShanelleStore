...
$price_range   = CatalogFilters::normalize_price_range( CatalogFilters::get_selected_price_range() );
?>
<form
					...
					>
						...
						<?php elseif ( 'radio' === $group['type'] ) : ?>
							<ul class="catalog-filters__options catalog-filters__options--radio">
								<?php
								$selected_values = CatalogFilters::get_selected_values( $group );
								$has_selection   = ! empty( $selected_values );
								?>
								<?php foreach ( $options as $option ) : ?>
									<?php
									...
									$checked  = '' === $slug
										? ! $has_selection
										: CatalogFilters::is_option_selected( $group, $slug );
									// Keep the clear option always visible.
									$hidden = '' !== $slug && (int) $option['index'] >= $visible_limit;
									?>
									<li class="catalog-filters__option<?php echo $hidden ? ' is-collapsed' : ''; ?>" data-filter-option>
