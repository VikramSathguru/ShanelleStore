 ...
 * @param {Record<string, unknown>} [options]
 */
function applyCartResponse( response, options = {} ) {
	if ( response?.cart ) {
	...
	document.body.dispatchEvent(
		new CustomEvent( 'shanelle:mini-cart:updated', {
			...
			detail: {
				...
				state: getCartState(),
				response,
				source: options.source || 'mini-cart',
			},
		} )
	);
}
...
async function updateCartItemQuantity( cartItemKey, quantity ) {
	...
	try {
		...
		if ( document.querySelector( '[data-shanelle-cart-page]' ) ) {
			payload.include_cart_page = '1';
		}

		const response = await requestCartAction( 'shanelle_mini_cart_update', payload );

		applyCartResponse( response, { source: 'mini-cart' } );
...
/**
 * Sync MiniCart state when the cart page mutates the session.
 *
 * Fragments are already applied by CartPage; only client state needs updating.
 *
 * @param {CustomEvent} event
 */
function handleCartPageUpdated( event ) {
...
function bindGlobalEvents() {
	...
	document.body.addEventListener( 'shanelle:cart-page:updated', handleCartPageUpdated );
}
