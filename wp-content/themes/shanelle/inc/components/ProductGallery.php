...
final class ProductGallery {
	...
	public static function render_main_image(): void {
		...
		<div class="product-gallery__stage" data-shanelle-gallery-stage>
			...
			<?php if ( $count > 1 ) : ?>
				<p
					class="product-gallery__position"
					data-shanelle-gallery-position
					aria-hidden="true"
				>
					<span data-shanelle-gallery-position-current>1</span>
					<span class="product-gallery__position-sep" aria-hidden="true"> / </span>
					<span data-shanelle-gallery-position-total><?php echo esc_html( (string) $count ); ?></span>
				</p>
			<?php endif; ?>
	...
	private static function build_gallery_items( \WC_Product $product ): array {
		...
		// Include variation-only images so color/size selection can sync the stage.
		if ( $product->is_type( 'variable' ) && $product instanceof \WC_Product_Variable ) {
			foreach ( $product->get_children() as $child_id ) {
				...
				if ( $variation_image_id > 0 && ! in_array( $variation_image_id, $attachment_ids, true ) ) {
