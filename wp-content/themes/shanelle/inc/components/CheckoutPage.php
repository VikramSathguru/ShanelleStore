...
final class CheckoutPage {
		...
		wp_localize_script(
			...
			array(
				...
				'i18n'         => array(
					...
					'placeOrder'      => apply_filters( 'woocommerce_order_button_text', __( 'Place order', 'woocommerce' ) ),
	...
	/**
	 * Render express / wallet payment presentation slot.
	 *
	 * Theme-owned wrapper around `woocommerce_checkout_before_customer_details`
	 * so gateway plugins (Stripe, PayPal, etc.) can inject buttons naturally.
	 * No gateway-specific logic — empty when plugins output nothing.
	 */
	public static function render_express_payments(): void {
		ob_start();
		do_action( 'woocommerce_checkout_before_customer_details' );
		$content = ob_get_clean() ?: '';

		$has_output = '' !== trim( $content );
		$classes    = 'checkout-page__express';

		if ( ! $has_output ) {
			$classes .= ' checkout-page__express--empty';
		}
		?>
		<div
			class="<?php echo esc_attr( $classes ); ?>"
			data-shanelle-checkout-express
			<?php echo $has_output ? '' : 'hidden'; ?>
		>
			<?php
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- WC / gateway plugin markup.
			echo $content;
			?>
		</div>
		<?php
	...
	public static function render_order_review(): void {
		...
		?>
		<div
			class="checkout-page__review woocommerce-checkout-review-order-table"
			data-shanelle-checkout-order-review
		>
	...
	/**
	 * Render mobile sticky place-order bar (total + trigger for WC #place_order).
	 *
	 * Presentation only: the button clicks WooCommerce's existing submit control.
	 * Hidden on desktop via CSS; JS hides it when #place_order is missing or on-screen.
	 */
	public static function render_sticky_place_order(): void {
		$order_button_text = apply_filters( 'woocommerce_order_button_text', __( 'Place order', 'woocommerce' ) );
		?>
		<div
			class="checkout-page__sticky"
			data-shanelle-checkout-sticky
			role="region"
			aria-label="<?php esc_attr_e( 'Realizar pedido', 'shanelle' ); ?>"
			hidden
		>
			<div class="checkout-page__sticky-inner">
				<div class="checkout-page__sticky-total">
					<span class="checkout-page__sticky-total-label text-label"><?php esc_html_e( 'Total', 'shanelle' ); ?></span>
					<span class="checkout-page__sticky-total-value text-price" data-shanelle-checkout-sticky-total>
						<?php echo wp_kses_post( self::get_order_total_html() ); ?>
					</span>
				</div>
				<button
					type="button"
					class="btn btn--primary btn--lg checkout-page__sticky-submit"
					data-shanelle-checkout-sticky-submit
				>
					<?php echo esc_html( (string) $order_button_text ); ?>
				</button>
			</div>
		</div>
		<?php
	...
	/**
	 * Resolve order total HTML from checkout totals rows (WC cart APIs).
	 */
	public static function get_order_total_html(): string {
		$totals = is_array( self::$state['totals'] ?? null )
			? self::$state['totals']
			: self::build_checkout_totals_rows();
		...
		foreach ( $totals as $row ) {
			...
			if ( 'total' === (string) ( $row['class'] ?? '' ) ) {
				return (string) ( $row['value_html'] ?? '' );
			}
		}

		if ( function_exists( 'WC' ) && WC()->cart ) {
			return (string) WC()->cart->get_total();
		}
	...
	public static function add_checkout_fragments( array $fragments ): array {
		...
		/*
		 * Override WooCommerce's default review-order table fragment so AJAX
		 * block()/replaceWith targets Shanelle markup (same selector WC uses
		 * for the update_order_review loading overlay).
		 */
		$fragments['.woocommerce-checkout-review-order-table'] = $review_html;
		$fragments['[data-shanelle-checkout-order-review]']     = $review_html;
