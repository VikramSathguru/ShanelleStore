...
final class MiniCart {
	...
	/**
	 * Build AJAX / fragment payload for MiniCart + header count.
	 *
	 * Optionally merges CartPage fragment + state in the same response so cart
	 * mutations need only one round-trip (P1-C1 / P1-C2 / P1-C6).
	 *
	 * @param bool $include_cart_page Whether to include cart page UI payload.
	 * @return array<string, mixed>
	 */
	public static function build_ui_payload( bool $include_cart_page = false ): array {
		self::$state = self::build_cart_state();
		...
		if ( function_exists( 'shanelle_cart_count_fragment' ) ) {
			$fragments = shanelle_cart_count_fragment( $fragments );
		}
		...
		if ( $include_cart_page ) {
			$page_payload = CartPage::build_ui_payload( false );

			$response['state']     = $page_payload['state'] ?? array();
			$response['fragments'] = array_merge(
				$response['fragments'],
				is_array( $page_payload['fragments'] ?? null ) ? $page_payload['fragments'] : array()
			);
		}
