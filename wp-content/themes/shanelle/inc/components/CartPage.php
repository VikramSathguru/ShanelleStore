...
final class CartPage {
	...
	public static function enqueue_assets(): void {
		...
		if ( self::should_enqueue_shipping_assets() ) {
			// WooCommerce cart.js owns calculator toggle, country/state sync and submit.
	...
	/**
	 * Render mobile sticky checkout bar (cart total + Ir a pagar).
	 *
	 * Lives outside the AJAX layout fragment so qty updates can refresh the
	 * total node without remounting the bar. Hidden on empty cart and desktop.
	 */
	public static function render_sticky_checkout(): void {
		if ( self::is_empty() ) {
			return;
		}

		$total_html   = self::get_order_total_html();
		$checkout_url = (string) ( self::$state['urls']['checkout'] ?? wc_get_checkout_url() );
		?>
		<div
			class="cart-page__sticky"
			data-shanelle-cart-page-sticky
			role="region"
			aria-label="<?php esc_attr_e( 'Ir a pagar', 'shanelle' ); ?>"
		>
			<div class="cart-page__sticky-inner">
				<div class="cart-page__sticky-total">
					<span class="cart-page__sticky-total-label text-label"><?php esc_html_e( 'Total', 'shanelle' ); ?></span>
					<span class="cart-page__sticky-total-value text-price" data-shanelle-cart-page-sticky-total>
						<?php echo wp_kses_post( $total_html ); ?>
					</span>
				</div>
				<a
					class="btn btn--primary btn--lg cart-page__sticky-checkout"
					href="<?php echo esc_url( $checkout_url ); ?>"
					data-shanelle-cart-page-sticky-checkout
				>
					<?php esc_html_e( 'Ir a pagar', 'shanelle' ); ?>
				</a>
			</div>
	...
	/**
	 * Render the sticky total value node for AJAX fragment refresh.
	 */
	public static function render_sticky_total(): void {
		?>
		<span class="cart-page__sticky-total-value text-price" data-shanelle-cart-page-sticky-total>
	...
	/**
	 * Resolve order total HTML from normalized totals rows (WC cart APIs).
	 */
	...
	/**
	 * Build AJAX / fragment payload for the cart page.
	 *
	 * When `$include_mini_cart` is true, merges MiniCart + header count fragments
	 * from the same cart session so one refresh keeps all bag surfaces in sync.
	 *
	 * @param bool $include_mini_cart Whether to include MiniCart UI payload.
	 * @return array<string, mixed>
	 */
	public static function build_ui_payload( bool $include_mini_cart = true ): array {
		self::$state = self::build_page_state();
		...
		ob_start();
		self::render_sticky_total();
		$sticky_total_html = ob_get_clean() ?: '';

		$response = array(
			'state'     => self::$state,
			'fragments' => array(
				'[data-shanelle-cart-page-fragment]'     => $fragment_html,
				'[data-shanelle-cart-page-sticky-total]' => $sticky_total_html,
			),
		...
		if ( $include_mini_cart ) {
			$mini_payload = MiniCart::build_ui_payload( false );

			$response['cart']      = $mini_payload['cart'] ?? array();
			$response['fragments'] = array_merge(
				is_array( $mini_payload['fragments'] ?? null ) ? $mini_payload['fragments'] : array(),
				$response['fragments']
			);
		}
