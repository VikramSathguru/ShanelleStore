...
final class CatalogFilters {
	...
	public static function get_filter_groups(): array {
		...
		foreach ( $groups as $index => $group ) {
			...
			// Category group with only the clear option has nothing useful to filter.
			if ( 'category' === (string) $group['id'] && count( $group['options'] ) <= 1 ) {
				continue;
			}
	...
	public static function get_group_options( array $group ): array {
		...
		if ( 'product_cat' === $taxonomy ) {
			$options[] = array(
				...
				'id'    => 0,
				'slug'  => '',
				'label' => self::get_category_clear_label(),
				'count' => 0,
				'color' => '',
			);
		}
	...
	/**
	 * Label for the category clear / “all” radio option.
	 */
	private static function get_category_clear_label(): string {
		if ( function_exists( 'is_product_category' ) && is_product_category() ) {
			return __( 'Todas en esta categoría', 'shanelle' );
		}

		return __( 'Todas las categorías', 'shanelle' );
	}
	...
	private static function build_price_meta_clauses(): array {
		$range = self::normalize_price_range( self::get_selected_price_range() );
		$clauses = array();
	...
	/**
	 * Ensure min <= max when both bounds are numeric.
	 *
	 * @param array{min: string, max: string} $range Price range.
	 * @return array{min: string, max: string}
	 */
	public static function normalize_price_range( array $range ): array {
		$min = $range['min'] ?? '';
		$max = $range['max'] ?? '';
	 ...
	 *
	 * Built from request params + filter definitions so every active URL filter is clearable.
	 *
	 * @return array<int, array<string, string>>
	 */
	public static function get_active_chips(): array {
		$chips         = array();
		$handled_params = array(
			self::QUERY_VAR_PREFIX . 'min_price' => true,
			self::QUERY_VAR_PREFIX . 'max_price' => true,
			self::QUERY_VAR_PREFIX . 'price'     => true,
		);

		foreach ( self::get_filter_definitions() as $group ) {
			if ( ! is_array( $group ) || 'price' === ( $group['type'] ?? '' ) ) {
				continue;
			}
			...
			foreach ( $selected as $slug ) {
				...
				$chips[] = array(
					'id'         => (string) ( $group['id'] ?? $param ) . ':' . $slug,
					'label'      => self::resolve_filter_value_label( $group, $slug ),
		...
		$price = self::normalize_price_range( self::get_selected_price_range() );
		...
		// Orphan shanelle_filter_* params (unknown roles) remain clearable.
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		foreach ( (array) wp_unslash( $_GET ) as $key => $raw ) {
			if ( ! is_string( $key ) || ! str_starts_with( $key, self::QUERY_VAR_PREFIX ) ) {
				continue;
			}

			if ( isset( $handled_params[ $key ] ) ) {
				continue;
			}

			$values = is_array( $raw )
				? array_values( array_filter( array_map( 'sanitize_title', $raw ) ) )
				: array_filter( array( sanitize_title( (string) $raw ) ) );
	...
	/**
	 * Resolve a human label for an active filter value.
	 *
	 * @param array<string, mixed> $group Filter group.
	 * @param string               $slug  Term slug.
	 */
	private static function resolve_filter_value_label( array $group, string $slug ): string {
		$taxonomy = (string) ( $group['taxonomy'] ?? '' );
