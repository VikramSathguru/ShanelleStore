...
final class ProductGrid {
		...
		<div class="product-grid product-grid--empty" data-shanelle-product-grid data-grid-state="empty">
			<div class="product-grid__empty">
				...
				<?php if ( ! empty( $actions ) ) : ?>
					<div class="product-grid__empty-actions">
						<?php foreach ( $actions as $action ) : ?>
							...
							?>
							<a
								class="btn <?php echo $is_primary ? 'btn--primary' : 'btn--outline'; ?> product-grid__empty-action"
	...
	/**
	 * Normalize empty-state action buttons.
	 *
	 * @return array<int, array<string, mixed>>
	 */
	private static function get_empty_actions(): array {
		$actions = self::$args['empty_actions'] ?? null;

		if ( is_array( $actions ) && ! empty( $actions ) ) {
			return array_values( $actions );
		}

		$shop = shanelle_is_woocommerce_active() ? wc_get_page_permalink( 'shop' ) : home_url( '/' );

		return array(
			array(
				'url'     => is_string( $shop ) ? $shop : home_url( '/' ),
				'label'   => __( 'Ver todos los productos', 'shanelle' ),
				'primary' => true,
			),
		);
	}
