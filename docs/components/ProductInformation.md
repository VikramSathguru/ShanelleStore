...
`ProductInformation` is theme-owned presentation. On the PDP it is composed by `ProductDetail` inside `.product-detail__commerce`, **after** the WooCommerce cart / variations form closes. It must not live inside the cart form and must not be duplicated by the legacy `ProductDetail::render_information_section()` placeholder.
...
On the product detail page (already wired by `ProductDetail::render_hero()` after the cart form):
