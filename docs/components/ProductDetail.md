...
- Compose ProductGallery, ProductSummary, ProductVariations, ProductPurchase, and ProductInformation
...
| `render_commerce_stack()` | Form-bound summary, variations, and purchase controls |
...
- ProductVariations
- ProductPurchase
- ProductInformation (theme-owned informational presentation; rendered after the cart form inside the commerce column)
...
`ProductDetail::render_hero()` owns the WooCommerce form boundary. The simple or variable cart form wraps only `ProductSummary`, `ProductVariations` (for variable products), and `ProductPurchase`. After the form closes, `ProductInformation` renders as its sibling inside `.product-detail__commerce`.

This keeps description, specifications, size, care, shipping, and returns content outside cart submission while preserving the existing commerce-column layout and ProductInformation accordion ownership.
