...
| `shanelle_mini_cart_update` | `MiniCart::ajax_update_item` | Update/remove line qty; returns MiniCart + header count; optional `include_cart_page=1` adds cart page fragment/state |
| `shanelle_mini_cart_get` | `MiniCart::ajax_get_cart` | Refresh drawer (+ header); optional `include_cart_page` |
| `shanelle_cart_page_get` | `CartPage::ajax_get_page` | Refresh cart page + MiniCart + header fragments |
