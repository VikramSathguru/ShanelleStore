...
| Field | Detail |
|-------|--------|
| Purpose | Card payments + express wallets (Apple Pay / Google Pay / Link when enabled) |
| Requirements | Stripe account, WooCommerce Stripe Gateway plugin (typical) |
| Dependencies | Classic checkout; `CheckoutPage` express slot fires `woocommerce_checkout_before_customer_details` (Stripe’s usual checkout hook) |
| Expected architecture | Official WC Stripe plugin; theme CSS only if needed for gateway UI |
| Theme must not | Embed Stripe.js, publishable keys, or PaymentIntent logic |
| Difficulty | Low (with official plugin) |
| Risks | SCA/3DS UX; webhook endpoints on Hostinger must be reachable; avoid hooking express only to `woocommerce_review_order_before_payment` (skipped during AJAX) |
...
| Field | Detail |
|-------|--------|
| Purpose | PayPal / Pay Later / Smart Buttons |
| Requirements | PayPal commerce plugin or WC PayPal Payments |
| Dependencies | WC classic checkout; express slot + `#payment` / `.payment_box` remain available |
| Expected architecture | Plugin gateway; avoid custom PayPal SDK in theme |
| Theme must not | Embed PayPal JS SDK or client IDs in composers |
| Difficulty | Low–Medium |
| Risks | Express button placement — prefer plugin defaults into `woocommerce_checkout_before_customer_details` or payment box; rehearsal on Shanelle layout before launch |
