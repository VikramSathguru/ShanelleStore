...
Cart form open/close is owned by `ProductDetail` (`variations_form` / simple `cart` form with nonces and hidden fields). The form contains only `ProductSummary`, `ProductVariations` (when applicable), and `ProductPurchase`. `ProductInformation` remains inside the commerce column but renders after the closing `</form>`; it owns informational presentation and is not part of WooCommerce cart submission.
...
| AJAX refresh | `wc_ajax_shanelle_cart_page_get`; qty/remove via `shanelle_mini_cart_update` (+ `include_cart_page`) |
...
| Express slot | `.checkout-page__express` wraps `woocommerce_checkout_before_customer_details` (gateway plugins inject here; empty when unused) |
| Review AJAX class | Theme review root includes `woocommerce-checkout-review-order-table` so WC `checkout.js` can block during `update_order_review` |
| Fragments | `woocommerce_update_order_review_fragments` — theme overrides `.woocommerce-checkout-review-order-table` with Shanelle markup |
| Checkout type | **Classic shortcode checkout required** — Checkout block bypasses theme overrides |

**Ownership:** WooCommerce owns validation, order creation, payment processing, shipping rates, and customer fields. The theme owns presentation only.
...
- Variation image sync: `shanelle:product-variations:gallery-change` → `ProductGallery` selects the matching attachment (variation images are included in gallery items). Reset clears to the first gallery image.  
- Native selects for matching live under a hidden `.variations` wrapper so WooCommerce `VariationForm` (`$attributeFields` / `change.wc-variation-form`) can sync `input.variation_id`. Custom swatches/chips set those selects via jQuery `.val().trigger('change')`.
